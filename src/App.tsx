...
function App() {
    ...
    reactChips.push(<div key={"not"} draggable={true} onDragStart={(e) => {
        e.dataTransfer.setData("newChip", "not")
    }}>
        <div className={"pointer-events-none"}>
            <Not not={{id: "not", leftPercent: 0, topPercent: 0, onMainCanvas: true, type: "not", value: false}}/>
        </div>
    </div>)
        ...
        <nodeContext.Provider value={{nodes, setNodes}}>
            <dragContext.Provider value={{drag, setDrag}}>
                <div className={"w-full h-screen bg-slate-900 p-8 flex"}>
                    ...
                    <div
                        className={"h-full w-[20vw] bg-green-800 flex flex-col items-center p-2 space-y-4 overflow-auto"}>
                        {reactChips}
                    </div>
                </div>
